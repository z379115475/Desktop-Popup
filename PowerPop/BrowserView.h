...
#define WM_BROWSENEWWIODOW			 (WM_APP + 2)
...
public:
	...
	BOOL KillHtmlScrollBars(CComPtr<IHTMLWindow2> spWnd2)
	{
		HRESULT hr;
		CComBSTR innerCode = TEXT("(function hideOverflow() { document.body.style.overflow='hidden'; var scrollheight=document.getElementsByTagName('html')[0];scrollheight.style.overflow= 'hidden';})();");

		CComPtr<IDispatchEx> dispexWindow;
		_S(spWnd2->QueryInterface(&dispexWindow));

		DISPID dispid = -1;
		_S(dispexWindow->GetDispID(CComBSTR("eval"), fdexNameCaseSensitive, &dispid));
		hr = disp_cast(dispexWindow).Invoke1(dispid, &CComVariant(innerCode));
			...
			if(SUCCEEDED(hRet) && spDocument != NULL)
			{
				...
				if(spHtmlDoc != NULL)
				{
					...
					if (spParentWindow != NULL) {
						KillHtmlScrollBars(spParentWindow);
...
// Event map and handlers
  ...
  #pragma warning(default:4867)
	...
	void __stdcall OnEventDocumentComplete(IDispatch* pDisp, VARIANT* URL)
	{
		...
		USES_CONVERSION;
		::PostMessage(GetParent(), WM_BROWSERDOCUMENTCOMPLETE, (WPARAM)m_hWnd, (LPARAM)OLE2T(URL->bstrVal));
		SetFocusToHTML();
	}
	...
	{
		USES_CONVERSION;
		::SendMessage(GetParent(), WM_BROWSERTITLECHANGE, (WPARAM)m_hWnd, (LPARAM)OLE2CT(Text));
	}

	void __stdcall OnEventNewWindow2(IDispatch **ppDisp, BOOL* pCancel)
	{
		::PostMessage(GetParent(), WM_BROWSENEWWIODOW, (WPARAM)m_hWnd, NULL);
	}
...
__declspec(selectany) _ATL_FUNC_INFO CBrowserView::NewWindow2_Info = { CC_STDCALL, VT_EMPTY, 2, { VT_DISPATCH, VT_BOOL | VT_BYREF } };
